...
bool isCollinear(Point p1, Point q1, Point p2) {
    if (orientation(p1, q1, p2) == 0 && onSegment(p1, p2, q1))
        return true;
    return false;
}
    ...
    for (size_t i = 0; i < segments.size(); ++i) {
        for (size_t j = i + 1; j < segments.size(); ++j) {
            if (doIntersect(segments[i], segments[j])) {
                if (isCollinear(segments[i].p1, segments[i].p2, segments[j].p1) ||
                    isCollinear(segments[i].p1, segments[i].p2, segments[j].p2) ||
                    isCollinear(segments[j].p1, segments[j].p2, segments[i].p1) ||
                    isCollinear(segments[j].p1, segments[j].p2, segments[i].p2)) {
                    count++;
                }
            }
        }
